...
__all__ = ["SynthFail","FaultTolerant","UnNormalize", "Compose", "ToTensor", "ToPILImage", "Normalize", "Resize", 
           "Lambda", "RandomHorizontalFlip", "RandomResizedCrop", "ColorJitter",
...
class ToTensor(object):
    ...
    def __call__(self, pic, ann=None, box=None):
        """
        Args:
        ...
        pic = F.to_tensor(pic)
        ann = F.to_tensor(ann)
        # ann = F.to_tensor(ann, False) if ann is not None else None
        # box = torch.from_numpy(box) if box is not None else None        
        box = torch.FloatTensor(box) if box is not None else None
...
class Normalize(object):
    ...
    def __call__(self, tensor, mask=None, box=None):
        """
        Args:
            tensor (Tensor): Tensor image of size (C, H, W) to be normalized.
        Returns:
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, ann=None, box=None):
        """
        Args:
...
class RandomResizedCrop(object):
    ...
    # def __init__(self, size, scale=(0.08, 1.0), ratio=(3./4., 4./3., 1.), interpolation=Image.BILINEAR):
    ...
    def get_params(img, scale, ratio, box):
        ...
        for attempt in range(50):
            ...
            # h = random.uniform(scale[0]*height, scale[1]*height)

            # aspect ratio constraint
            area_scale = (h*w) / area
            # if h / w < ratio[0] or h / w > ratio[1] or area_scale < scale[0] or area_scale > scale[1]:
            ...
            # w = width if i < 0 else w
            # h = height if j < 0 else h
            # # convert to integer rect x1,y1,x2,y2
            # rect = np.array([int(i), int(j), int(i+w), int(j+h)])
            ...
            # box_validity = F.box_is_valid(box, i, j, j+h, i+w, img.size)
            ...
            # if check_validity:
            #     if not F.box_is_valid(box, i, j, h, w, img.size):
            #         continue
            #     else:
            #         return i, j, h, w
...
class ColorJitterLWIR(object):
    ...
    def get_params(brightness, contrast, saturation, hue):
        ...
        if brightness is not None:
            ...
            transforms.append(Lambda(lambda img, mask, boxes: (img, F.adjust_brightness(mask, brightness_factor), boxes) ))

        if contrast is not None:
            contrast_factor = random.uniform(contrast[0], contrast[1])
            transforms.append(Lambda(lambda img, mask, boxes: (img, F.adjust_contrast(mask, contrast_factor), boxes) ))

        if saturation is not None:
            saturation_factor = random.uniform(saturation[0], saturation[1])
            transforms.append(Lambda(lambda img, mask, boxes: (img, F.adjust_saturation(mask, saturation_factor), boxes) ))

        if hue is not None:
            hue_factor = random.uniform(hue[0], hue[1])
            transforms.append(Lambda(lambda img, mask, boxes: (img, F.adjust_hue(mask, hue_factor), boxes) ))
    ...
    def __call__(self, img, mask=None, boxes=None):
        ...
        return transform(img, mask, boxes)
...
class ColorJitter(object):
    ...
    def get_params(brightness, contrast, saturation, hue):
        ...
        if brightness is not None:
            brightness_factor = random.uniform(brightness[0], brightness[1])
            transforms.append(Lambda(lambda img, mask, boxes: (F.adjust_brightness(img, brightness_factor), mask, boxes) ))

        if contrast is not None:
            contrast_factor = random.uniform(contrast[0], contrast[1])
            transforms.append(Lambda(lambda img, mask, boxes: (F.adjust_contrast(img, contrast_factor), mask, boxes) ))

        if saturation is not None:
            saturation_factor = random.uniform(saturation[0], saturation[1])
            transforms.append(Lambda(lambda img, mask, boxes: (F.adjust_saturation(img, saturation_factor), mask, boxes) ))

        if hue is not None:
            hue_factor = random.uniform(hue[0], hue[1])
            transforms.append(Lambda(lambda img, mask, boxes: (F.adjust_hue(img, hue_factor), mask, boxes) ))
