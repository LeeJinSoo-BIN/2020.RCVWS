...
checkpoint = './jobs/2020-02-24_12h36m_train_multi_4_cls_hard/checkpoint_ssd300.pth.tar015'
...
test_dataset = Sejong_Ped('train.txt',img_transform=preprocess1, co_transform=transforms1, condition='test')
...
def detect(original_vis, original_lwir, boxes, true_labels, min_score, max_overlap, top_k, suppress=None):
    ...
    for i in range(true_bboxes_out.size(0)):
        ...
        test_true_location = [box_location_true[0] + 2., box_location_true[1] - text_true_size[1]]
        ...
        draw.text(xy=test_true_location, text='{:s}'.format('GT_'+true_label_map), fill='white', font=font)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for (ii, (image_vis, image_lwir, boxes, labels, index)) in tqdm(zip(ids, test_loader), desc='Drawing'):
            ...
            annotate_lwir = detect(ori_vis, ori_lwir, boxes[0], labels,  min_score=0.8, max_overlap=0.45, top_k=200)

            annotate_lwir.save('./Detection_visualization_hard_occ/{:06d}.jpg'.format(img_id))
            img_id = img_id+1
